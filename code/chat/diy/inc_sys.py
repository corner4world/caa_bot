...
#15 加解密
def secret_ldae(text_p,secret_if="yes",key_p="ldaeisgood",salt_p = b'llddaaeeiissookk'):
...
def str_is(str_p):
    ...
    # 识别手机号
    pass_if = re.findall(r"1\d{10}",str_p)    
    if (pass_if):  
        it_is = "mp"
        return it_is
    ...
    # 识别电子邮件
    if (len(str_p) > 7):
        if (re.match("^.+\\@(\\[?)[a-zA-Z0-9\\-\\.]+\\.([a-zA-Z]{2,3}|[0-9]{1,3})(\\]?)$", str_p) != None):
            it_is = "email"
            return it_is
            
    # 识别QQ号
    regex = re.compile(r'[1-9]\d{4,10}', re.IGNORECASE) 
    pass_if = re.findall(regex,str_p)
    if (pass_if):  
        it_is = "qq"
        return it_is
