...
def run_it(**args):
    ...
    if (action_p == "cf_lr_yesno"):
        ...
        rate_t = -1 # 意图分类值
        step_last = 1 # 最佳迭代次数
        name_table_p = "feature_train_txt"
        ...
        print ("数据元组",row) # 测试用
        try:
            dic_d = feature_base.dim_get_stand(row_p=row,which_p=which,name_table_p=name_table_p,rows_avg=rows_avg,rows_std=rows_std,numb_dim=dim_f)
        except:
            pass
        ...
        rate_t = feature_dae.what_class_lr(list_last=list_w,path_dae_p=path_dae,dim_p=2,test_p=0)
        dic_t = {"classify":rate_t}
        return dic_t
    ...
    def extract_answer_similar(content_p="",key_p=""):
        ...
        j = 1
        for x in list_order:
            txt += "\"" + str(j) + "\"" + ":['" + dic_sen_all[x[0]][0] + "'," + str(x[1]) + "," + str(dic_sen_all[x[0]][1]) + "," + str(dic_sen_all[x[0]][2]) + "],\n"
            j += 1
        txt = txt[:-1]
        txt = "{" + txt + "}"
